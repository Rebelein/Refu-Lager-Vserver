...
export function createCrudRouter(model) {
    ...
    router.get('/', async (_req, res, next) => {
        try {
            ...
            const out = docs.map((d) => ({ ...d, id: d._id, _id: undefined }));
            res.json(out);
        }
        catch (e) {
            next(e);
    ...
    router.put('/:id', async (req, res, next) => {
        try {
            ...
            delete payload.id;
            const doc = await model.findByIdAndUpdate(req.params.id, { $set: payload }, { upsert: true, new: true }).lean({ virtuals: true });
            const out = { ...doc, id: doc._id };
            delete out._id;
            res.json(out);
        }
